...
public class GPSLLdeg2UTM extends Form implements CommandListener{
    ...
    private TextField LondegdField, LondegmField , LondegcField, LatdegdField, LatdegmField , LatdegcField;

	public GPSLLdeg2UTM(String title,UIController control){
		super(title);
    	controller=control;
    	...
    	LondegmField=new TextField("'","",2,TextField.DECIMAL);
    	LondegcField=new TextField("\"","",5,TextField.DECIMAL);
    	
    	LatdegdField=new TextField("Latitude: °","",5,TextField.DECIMAL);
    	LatdegmField=new TextField("'","",2,TextField.DECIMAL);
    	LatdegcField=new TextField("\"","",5,TextField.DECIMAL);
    	
    	
    	this.append(LondegdField);
    	this.append(LondegmField);
    	this.append(LondegcField);
    	this.append(LatdegdField);
    	this.append(LatdegmField);
    	this.append(LatdegcField);

	}
	public void clear(){
		LondegdField.setString("");
		LondegmField.setString("");
		LondegcField.setString("");
		LatdegdField.setString("");
		LatdegmField.setString("");
		LatdegcField.setString("");
	    	}
    /*
	    * LLTOUTM_Execute
	    *
	...
	private void  LLToUTM_Execute (String londec, String latdec)
	    {
	        ...
	        if ((lon < -180.0) || (180.0 <= lon)) {
	        	alerterror = new AlertError(Setting.TITLE_AlertError, "The longitude you entered is out of range.  \n" +
		                   "Please enter a number in the range [-180°00'00\", 180°00'00\").");
                controller.setCurrent(alerterror,this);
	            return;
	        }
	        ...
	        if ((lat < -90.0) || (90.0 < lat)) {
	        	alerterror = new AlertError(Setting.TITLE_AlertError, "The latitude you entered is out of range.  \n" +
		                   "Please enter a number in the range [-90°00'00\", 90°00'00\"].");
                controller.setCurrent(alerterror,this);
	            return;
	        }
    ...
    private void CalAlert(double x, double y, int zone, String NS){
    	...
    	public void commandAction(Command command, Displayable disp){
    			if(command==backCommand){
    	            ...
    	            String londegc=LondegcField.getString();
    	            String latdegd=LatdegdField.getString();
    	            String latdegm=LatdegmField.getString();
    	            String latdegc=LatdegcField.getString();
    	            
    	            if(londegd.length()==0 || londegm.length()==0 || londegc.length()==0 || latdegd.length()==0 || latdegm.length()==0 || latdegc.length()==0){	
    	                alerterror = new AlertError(Setting.TITLE_AlertError, "Please enter a valid degree value in the dms field.");
    	                controller.setCurrent(alerterror,this);
    	 	            	return;
    	 	            }
    	            gpscalfun = new GPSCalFunc();
    	            double londec = gpscalfun.deg2dec(londegd, londegm, londegc);
    	            double latdec = gpscalfun.deg2dec(latdegd, latdegm, latdegc);
